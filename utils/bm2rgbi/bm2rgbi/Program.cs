...
namespace bm2rgbi
{
    class Program
    {
        ...
        public void run(string[] args_)
        {
            ...
            try
            {
                ...
                using (FileStream fs = new FileStream(args.OutputBitmapFilename, FileMode.Create, FileAccess.Write, FileShare.None))
                    args.TargetDevice.convert(bm, fs, args.ByteOrder);
                ...
                export(args.OutputBitmapFilename);
        ...
        /// <summary>
        /// Export result into C file
        /// </summary>

        private void export(string filename)
        {
            string procpath, arrayfile;
            ...
            psi = new ProcessStartInfo(Path.Combine(procpath, "xxd.exe"), "-i " + filename + " " + arrayfile);
            psi.CreateNoWindow = true;
            psi.WindowStyle = ProcessWindowStyle.Hidden;

            if ((p = Process.Start(psi)) == null)
                throw new Exception("Failed to start C export process (xxd.exe)");
            ...
            fi = new FileInfo(arrayfile);
            if (fi.Length == 0)
                throw new Exception("The C export process (xxd.exe) failed");
