...
namespace stm32plus {
  namespace display {
    ...
    /**
     * Template class holding the specialisation of SSD1289Colour for 18-bit colours
     * @tparam TAccessMode The access mode class (e.g. FSMC)
     */

    template<class TAccessMode>
    class SSD1289Colour<COLOURS_18BIT,TAccessMode> {

      private:
        TAccessMode& _accessMode;

      protected:
        SSD1289Colour(TAccessMode& accessMode);
    ...
    template<class TAccessMode>
    inline SSD1289Colour<COLOURS_18BIT,TAccessMode>::SSD1289Colour(TAccessMode& accessMode)
    ...
    template<class TAccessMode>
    constexpr inline uint16_t SSD1289Colour<COLOURS_18BIT,TAccessMode>::getColourEntryMode() const {
      return 0x4040;      // 262K and type B: RRRRRRxxGGGGGGxx,xxxxxxxxBBBBBBxx
    }
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::unpackColour(tCOLOUR src,UnpackedColour& dest) const {

      dest.first=static_cast<uint16_t>((src & 0x00fc0000) >> 8) | ((src & 0x0000fc00) >> 8);
      dest.second=static_cast<uint16_t>(src & 0x000000fc);
    }
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::unpackColour(uint8_t red,uint8_t green,uint8_t blue,UnpackedColour& dest) const {

      dest.first=(static_cast<uint16_t>(red & 0xfc) << 8) | (green & 0xfc);
      dest.second=blue & 0xfc;
    }
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::writePixel(const UnpackedColour& cr) const {
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::writePixelAgain(const UnpackedColour& cr) const {
      _accessMode.writeData(cr.first);
      _accessMode.writeData(cr.second);
    }
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::fillPixels(uint32_t numPixels,const UnpackedColour& cr) const {
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::allocatePixelBuffer(uint32_t numPixels,uint8_t*& buffer,uint32_t& bytesPerPixel) const {
    ...
    template<class TAccessMode>
    inline void SSD1289Colour<COLOURS_18BIT,TAccessMode>::rawTransfer(const void *buffer,uint32_t numPixels) const {
