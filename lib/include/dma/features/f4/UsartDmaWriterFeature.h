...
namespace stm32plus {
  ...
  inline UsartDmaWriterFeature<TUsart,TPriority,TFifoMode>::UsartDmaWriterFeature(Dma& dma)
    : DmaFeatureBase(dma) {
  ...
  template<class TUsart,uint32_t TPriority,uint32_t TFifoMode>
  inline void UsartDmaWriterFeature<TUsart,TPriority,TFifoMode>::beginWrite(const void *source,uint32_t count) {
