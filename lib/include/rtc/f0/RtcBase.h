...
namespace stm32plus {
  ...
  inline RtcBase::RtcBase(uint32_t hourFormat, uint32_t backupValue)
    ...
    // If the LSE is not connected properly, the backup register can be set
    // without being able to set the time. Also reset clock if the
    // date and time are all zeros.
