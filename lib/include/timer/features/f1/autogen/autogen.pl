...
sub writeFeature {
		...
		if(length($noremap)>0) { $portNone="GPIO" . substr($noremap,1,1); } else { $portNone="nullptr"; }
		if(length($remap1)>0) { $port1="GPIO" . substr($remap1,1,1); } else { $port1="nullptr"; }
		if(length($remap2)>0) { $port2="GPIO" . substr($remap2,1,1); } else { $port2="nullptr"; }
		if(length($remapf)>0) { $portf="GPIO" . substr($remapf,1,1); } else { $portf="nullptr"; }
...
~;
		...
		if($mode eq "GPIO_Mode_IN_FLOATING") {
				print {$outfile} qq/
      GpioPinInitialiser::initialise(ports[TRemapLevel],pins[TRemapLevel],Gpio::INPUT,(GPIOSpeed_TypeDef)PeripheralTraits<PERIPHERAL_TIMER${timerNumber}>::GPIO_SPEED,Gpio::PUPD_NONE); /;
		}
