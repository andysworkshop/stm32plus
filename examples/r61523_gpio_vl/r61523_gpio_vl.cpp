 ...
 * R61523 display panel. The panel in this demo is from the Sony Ericsson U5 Vivaz
 * cellphone (see my website for writeup). The access mode is the
 * highly optimised GPIO mode in 64K colours, landscape mode.
 *
 * This is designed to run on the 24MHz STM32 Value Line Discovery board although it should
 * work on any device with a 24MHz or slower HCLK as long as you've got the startup
 * parameters for the clocks and the linker script for the memory sizing set up correctly.
 *
 * The pinout is changeable to your needs. The demo setup is as follows.
 *
 * LCD_D0..15 = PB[0..15]       // a whole port is used for the 16 data lines
 * LCD RESET  = PA0             // RESET/WR/RS must be on the same port
 * LCD_WR     = PA1
 * LCD_RS     = PA2
 *
...
class R61523Test {
  ...
  public:
    ...
    /**
     * Gpio16AccessMode is templated with the R61523Test class. Therefore it will expect to
     * find the following constants available for static access
     */

    enum {
      Port_CONTROL = GPIOA_BASE,        // will use [0..2]
      Port_DATA    = GPIOB_BASE,        // will use whole port as data bus D0..15
  ...
  protected:
    typedef Gpio16BitAccessMode_R61523_64K<R61523Test> LcdAccessMode;
