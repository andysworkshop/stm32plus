 ...
 * data is sent from SPI1 to SPI2 and, if successfully received, then a LED on PF6 is
 * flashed for 1 second. The test repeats continuously.
 *
 * The SPI interrupts are used to send and receive the data. Note the code that adjusts the
 * relative interrupt priorities.
 *
 * If you intend to run this example on the STM23F4DISCOVERY board then replace PF6 with PD13
 * to use the onboard LED.
 *
 * If you intend to run this example on the STM23VLDISCOVERY board then replace PF6 with PC8 to
 * use the onboard LED.
 *
 * This demo is not compatible with the F0 because only SPI1 on the F0 supports interrupts. See
 * the spi_send_interrupts example for a demo that sends using interrupts and receives
 * synchronously.
 *
 * For this demo I'm going to need you to do a little wiring to hook up SPI1 to SPI2 so that
 * we can exchange data over the MOSI pin. Here's the connections that you need to make.
 *
...
class SpiSendReceiveInterruptsTest {
  ...
  public:
    ...
    void run() {
      ...
      _sender->SpiInterruptEventSender.insertSubscriber(
          SpiInterruptEventSourceSlot::bind(this,&SpiSendReceiveInterruptsTest::onInterrupt)
        );
...
int main() {
  ...
  SpiSendReceiveInterruptsTest test;
  test.run();
