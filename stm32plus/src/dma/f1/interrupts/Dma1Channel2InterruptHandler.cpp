   ...
   */
    ...
    void __attribute__ ((interrupt("IRQ"))) DMA1_Channel2_IRQHandler() {
      ...
      if(DMA_GetITStatus(DMA1_IT_TC2)!=RESET) {
        DmaInterruptFeature<1,2>::_dmaInstance->notifyObservers(ObservableEvent::DMA_TransferComplete,(void *)0x00010002);
        DMA_ClearITPendingBit(DMA1_IT_TC2);
      }
      else if(DMA_GetITStatus(DMA1_IT_HT2)!=RESET) {
        DmaInterruptFeature<1,2>::_dmaInstance->notifyObservers(ObservableEvent::DMA_HalfTransfer,(void *)0x00010002);
        DMA_ClearITPendingBit(DMA1_IT_HT2);
      }
      else if(DMA_GetITStatus(DMA1_IT_TE2)!=RESET) {
        DmaInterruptFeature<1,2>::_dmaInstance->notifyObservers(ObservableEvent::DMA_TransferError,(void *)0x00010002);
