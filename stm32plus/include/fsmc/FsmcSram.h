...
namespace stm32plus {
  ...
  inline FsmcSram<TFsmc>::FsmcSram(const FsmcSramTiming& timing,uint32_t lastAddressLineIndex) {
    ...
    if(lastAddressLineIndex>=10)
      GpioPinInitialiser::initialise(GPIOG,Sram_pgmap[lastAddressLineIndex-10],Gpio::ALTERNATE_FUNCTION,(GPIOSpeed_TypeDef)FsmcPeripheralTraits::GPIO_SPEED,Gpio::PUPD_NONE,Gpio::PUSH_PULL,GPIO_AF_FSMC);

    // A0..A9 are on port F

    GpioPinInitialiser::initialise(GPIOF,Sram_pfmap[lastAddressLineIndex>9 ? 9 : lastAddressLineIndex],Gpio::ALTERNATE_FUNCTION,(GPIOSpeed_TypeDef)FsmcPeripheralTraits::GPIO_SPEED,Gpio::PUPD_NONE,Gpio::PUSH_PULL,GPIO_AF_FSMC);

    // A19..A23 are on port E

    if(lastAddressLineIndex>=19)
      GpioPinInitialiser::initialise(GPIOE,Sram_pemap[lastAddressLineIndex-19],Gpio::ALTERNATE_FUNCTION,(GPIOSpeed_TypeDef)FsmcPeripheralTraits::GPIO_SPEED,Gpio::PUPD_NONE,Gpio::PUSH_PULL,GPIO_AF_FSMC);

    // A16..A18 are on port D

    if(lastAddressLineIndex>=16)
      GpioPinInitialiser::initialise(GPIOD,Sram_pdmap[(lastAddressLineIndex>18 ? 18 : lastAddressLineIndex)-16],Gpio::ALTERNATE_FUNCTION,(GPIOSpeed_TypeDef)FsmcPeripheralTraits::GPIO_SPEED,Gpio::PUPD_NONE,Gpio::PUSH_PULL,GPIO_AF_FSMC);
