...
namespace stm32plus {
  ...
  /**
   * Base class for the DMA feature to handle interrupts. This class is the
   * only Observable in the DMA class suite.
   * @tparam TDmaNumber The number of the DMA peripheral (1..2)
   * @tparam TChannelNumber The number of the DMA channel (1..7) or (1..5)
   */

  template<uint8_t TDmaNumber,uint8_t TChannelNumber>
  class DmaInterruptFeature : public Observable, public DmaFeatureBase {
    ...
    public:
      static Observable *_dmaInstance;
  ...
  template<uint8_t TDmaNumber,uint8_t TChannelNumber>
  Observable *DmaInterruptFeature<TDmaNumber,TChannelNumber>::_dmaInstance;
